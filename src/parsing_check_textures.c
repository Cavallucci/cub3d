...
{	
	...
	d->door->img = mlx_xpm_file_to_image(d->mlx->mlx_ptr, pars->door[1], &(d->door->img_width), &(d->door->img_height));
	...
	if (!d->north->img || !d->south->img || !d->west->img || !d->east->img || !d->door->img)
		ft_free_close_error("Error\nPath textures", pars);
	d->door->mlx_ptr = mlx_get_data_addr(d->door->img, &d->door->pixel_bits, &d->door->line_bytes, &d->door->endian);	
